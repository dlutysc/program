 ...
 * 超时得原因可能是用字符串来构建图，就是图得节点是字符串。改进之后用字符串得索引来构建图，就是索引作为图得节点
 *
 * 这是一种思想：存索引，而不存具体的值
 *
 */
...
//        test();
    ...
    public static void test1(){
        ...
        int res = ladderLength1(begin, end, wordList);
        System.out.println(System.currentTimeMillis() - start); //使用索引构建图比用字符串构建图块140ms左右
        System.out.println(res);
    }
    public static void test(){
        String begin = "hit";
        String end = "cog";
        String[] strings = {"cog","dot","dog","lot","log","hot"};
        List<String> wordList = new ArrayList<>(Arrays.asList(strings));
        int res1 = ladderLength1(begin, end, wordList);
    ...
    public static int ladderLength(String beginWord, String endWord, List<String> wordList) {
        ...
        List<String>[] graph = buildGraph(wordList);

        int dictLen = wordList.size();
        boolean[] mark = new boolean[dictLen];
        Queue<String> q = new LinkedList<>();
        q.offer(beginWord);
        mark[wordList.indexOf(beginWord)] = true;
        int pathLen = 0;
        while (!q.isEmpty()) {
            int size = q.size();
            ++pathLen;
            while (size-- > 0){
                String cur = q.poll();
                for (String s : graph[wordList.indexOf(cur)]){
                    if (mark[wordList.indexOf(s)] == false){
                        q.offer(s);
                        mark[wordList.indexOf(s)] = true;
                    }
                    if (s.equals(endWord)){
                        return pathLen + 1;
                    }
    ...
    public static int ladderLength1(String beginWord, String endWord, List<String> wordList){
        ...
        List<Integer>[] graph = buildGraph1(wordList);
        /**
         *  @Author chasein
         *  @Date 2020/7/21 11:08
         *  @Description 用索引构建图,构建完成之后的样子
         *     1 ——5——6
         *     | \ |
         *     2   3
         *     | \ |
    ...
    static boolean isNbr(String str1, String str2){
     ...
     * @return
     */
    static List<String>[] buildGraph(List<String> dict){
        List<String>[] graph = new List[dict.size()];
        for (int i = 0; i < dict.size(); i++) {
            graph[i] = new ArrayList<>();
            for (int j = 0; j < dict.size(); j++) {
                if (isNbr(dict.get(i), dict.get(j))){
                    graph[i].add(dict.get(j));
                }
            }
        }
     ...
     * @return
     */
    static List<Integer>[] buildGraph1(List<String> dict){
        List<Integer>[] graph = new List[dict.size()];
        for (int i = 0; i < dict.size(); i++) {
            graph[i] = new ArrayList<>();
            for (int j = 0; j < dict.size(); j++) {
                if (isNbr(dict.get(i), dict.get(j))){
